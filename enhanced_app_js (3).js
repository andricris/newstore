function renderVoucherCard(voucher) {
    ...
    const isActive = voucher.is_active && !isExpired && !isNotStarted && !isUsageLimitReached;
    
    console.log('🎨 Rendering voucher card for:', voucher.id, voucher.code);
...
function renderProductRow(product) {
    console.log('🎨 Rendering product row for:', product.id, product.name);
        ...
        <tr class="border-b border-slate-100 hover:bg-slate-50">
            ...
            <td class="py-4 px-4">
                <div class="flex items-center">
                    ...
                    <button type="button" 
                            class="toggle-button-${product.id} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 ${product.is_available ? 'bg-green-600' : 'bg-gray-200'}"
                            onclick="handleProductToggle(${product.id})"
                            aria-label="Toggle product availability">
                        <span class="sr-only">Toggle product status</span>
                        <span class="toggle-thumb-${product.id} inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${product.is_available ? 'translate-x-6' : 'translate-x-1'}"></span>
                    </button>
                    <span class="toggle-label-${product.id} ml-3 text-sm font-medium ${product.is_available ? 'text-green-600' : 'text-gray-500'}">
                        ${product.is_available ? 'Tersedia' : 'Habis'}
                    </span>
                </div>
            </td>
...
function updateProductToggleUI(productId, newStatus) {
    console.log('🎨 Updating product toggle UI:', productId, 'newStatus:', newStatus);
    
    // Update button background
    const toggleButton = document.querySelector(`.toggle-button-${productId}`);
    if (toggleButton) {
        toggleButton.className = `toggle-button-${productId} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 ${newStatus ? 'bg-green-600' : 'bg-gray-200'}`;
        console.log('✅ Button updated');
    } else {
        console.error('❌ Toggle button not found:', `.toggle-button-${productId}`);
    }
    
    // Update thumb position
    const toggleThumb = document.querySelector(`.toggle-thumb-${productId}`);
    if (toggleThumb) {
        toggleThumb.className = `toggle-thumb-${productId} inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${newStatus ? 'translate-x-6' : 'translate-x-1'}`;
        console.log('✅ Thumb updated');
    } else {
        console.error('❌ Toggle thumb not found:', `.toggle-thumb-${productId}`);
    }
    
    // Update label
    const toggleLabel = document.querySelector(`.toggle-label-${productId}`);
    if (toggleLabel) {
        toggleLabel.textContent = newStatus ? 'Tersedia' : 'Habis';
        toggleLabel.className = `toggle-label-${productId} ml-3 text-sm font-medium ${newStatus ? 'text-green-600' : 'text-gray-500'}`;
        console.log('✅ Label updated');
    } else {
        console.error('❌ Toggle label not found:', `.toggle-label-${productId}`);
    }
}
...
function renderVoucherCard(voucher) {
    ...
    return `
        <div class="bg-white border border-slate-200 rounded-lg p-4 ${isActive ? 'border-green-200 bg-green-50' : ''}">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    ...
                    <div class="text-sm text-slate-500 space-y-1">
                        ...
                        ${voucher.usage_limit ? `<p>📈 Digunakan: ${voucher.used_count || 0}/${voucher.usage_limit}</p>` : `<p>📈 Digunakan: ${voucher.used_count || 0} kali</p>`}
                        ${voucher.valid_until ? `<p>⏰ Berlaku sampai: ${new Date(voucher.valid_until).toLocaleDateString('id-ID')}</p>` : ''}
                    </div>
                </div>
                
                <div class="flex items-center space-x-3">
                    <!-- Simple Toggle Switch for Voucher -->
                    <button type="button" 
                            class="toggle-button-voucher-${voucher.id} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 ${voucher.is_active ? 'bg-green-600' : 'bg-gray-200'}"
                            onclick="handleVoucherToggle(${voucher.id})"
                            aria-label="Toggle voucher status">
                        <span class="sr-only">Toggle voucher status</span>
                        <span class="toggle-thumb-voucher-${voucher.id} inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${voucher.is_active ? 'translate-x-6' : 'translate-x-1'}"></span>
                    </button>
...
// Add debugging function to test toggle switches
function debugToggleSwitches() {
    console.log('🔍 DEBUG: Toggle Switches Analysis');
    console.log('=====================================');
    
    // Check products
    const productToggles = document.querySelectorAll('[class*="toggle-button-"]:not([class*="voucher"])');
    console.log('🛍️ Product toggles found:', productToggles.length);
    productToggles.forEach((toggle, index) => {
        console.log(`  ${index + 1}. ${toggle.className}`);
        console.log(`     onclick: ${toggle.onclick}`);
        console.log(`     innerHTML: ${toggle.innerHTML}`);
    });
    
    // Check vouchers
    const voucherToggles = document.querySelectorAll('[class*="toggle-button-voucher-"]');
    console.log('🎟️ Voucher toggles found:', voucherToggles.length);
    voucherToggles.forEach((toggle, index) => {
        console.log(`  ${index + 1}. ${toggle.className}`);
        console.log(`     onclick: ${toggle.onclick}`);
    });
    ...
    console.log('  handleProductToggle:', typeof window.handleProductToggle);
    console.log('  handleVoucherToggle:', typeof window.handleVoucherToggle);
    
    // Check appState
    console.log('📊 AppState check:');
    console.log('  products length:', appState.products.length);
    console.log('  vouchers length:', appState.vouchers.length);
    console.log('  supabaseClient:', !!supabaseClient);
    
    console.log('=====================================');
}

// Test function for manual toggle
function testProductToggle(productId) {
    console.log('🧪 TEST: Manual product toggle for ID:', productId);
    handleProductToggle(productId);
}

function testVoucherToggle(voucherId) {
    console.log('🧪 TEST: Manual voucher toggle for ID:', voucherId);
    handleVoucherToggle(voucherId);
}
...
// Enhanced toggle handlers with better debugging
async function handleProductToggle(productId) {
    console.log('🔄 Product toggle clicked - ID:', productId);
    console.log('📊 Current appState.products:', appState.products);
    ...
    try {
        ...
        if (!product) {
            ...
            console.log('Available product IDs:', appState.products.map(p => p.id));
            return;
        }
        
        const currentStatus = product.is_available;
        const newStatus = !currentStatus;
        console.log(`📝 Changing status from ${currentStatus} to ${newStatus}`);
        
        // Update UI immediately
        console.log('🎨 Updating UI...');
        updateProductToggleUI(productId, newStatus);
        ...
        const product = appState.products.find(p => p.id == productId);
        if (product) {
            console.log('🔄 Reverting UI to original status:', product.is_available);
            updateProductToggleUI(productId, product.is_available);
        }
    }
}

// Enhanced voucher toggle with same pattern as products
async function handleVoucherToggle(voucherId) {
    console.log('🔄 Voucher toggle clicked - ID:', voucherId);
    ...
    try {
        ...
        const newStatus = !currentStatus;
        console.log(`📝 Changing voucher status from ${currentStatus} to ${newStatus}`);
        
        // Update UI immediately
        updateVoucherToggleUI(voucherId, newStatus);
        ...
        console.error('❌ Voucher toggle error:', error);
        showToast('Gagal mengubah status voucher: ' + error.message, 'error');
        
        // Revert UI changes on error
        const voucher = appState.vouchers.find(v => v.id == voucherId);
        if (voucher) {
            updateVoucherToggleUI(voucherId, voucher.is_active);
        }
    }
}

function updateVoucherToggleUI(voucherId, newStatus) {
    console.log('🎨 Updating voucher toggle UI:', voucherId, 'newStatus:', newStatus);
    
    // Update button background
    const toggleButton = document.querySelector(`.toggle-button-voucher-${voucherId}`);
    if (toggleButton) {
        toggleButton.className = `toggle-button-voucher-${voucherId} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 ${newStatus ? 'bg-green-600' : 'bg-gray-200'}`;
        console.log('✅ Voucher button updated');
    }
    
    // Update thumb position
    const toggleThumb = document.querySelector(`.toggle-thumb-voucher-${voucherId}`);
    if (toggleThumb) {
        toggleThumb.className = `toggle-thumb-voucher-${voucherId} inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${newStatus ? 'translate-x-6' : 'translate-x-1'}`;
        console.log('✅ Voucher thumb updated');
    }
    
    // Update voucher card styling
    const voucherCard = toggleButton?.closest('.bg-white');
    if (voucherCard) {
        if (newStatus) {
            voucherCard.classList.add('border-green-200', 'bg-green-50');
        } else {
            voucherCard.classList.remove('border-green-200', 'bg-green-50');
        }
        console.log('✅ Voucher card styling updated');
    }
}
...
async function toggleVoucherStatus(voucherId, toggleElement) {
    console.log('🔄 Toggling voucher status:', voucherId);
    ...
    try {
        ...
        console.log(`📝 Updating voucher ${voucherId} status to:`, newStatus);
        
        // Optimistically update UI first
        if (toggleElement) {
            const switchContainer = toggleElement.closest('.relative');
            const toggleBall = switchContainer.querySelector('div:last-child');
            const switchTrack = switchContainer.querySelector('div:first-child');
            
            // Update switch appearance immediately
            toggleElement.checked = newStatus;
            if (newStatus) {
                switchTrack.className = 'w-10 h-6 rounded-full transition-colors bg-green-500';
                toggleBall.className = 'absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform translate-x-4';
            } else {
                switchTrack.className = 'w-10 h-6 rounded-full transition-colors bg-slate-300';
                toggleBall.className = 'absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform translate-x-0';
            }
            
            // Update voucher card styling
            const voucherCard = toggleElement.closest('.bg-white');
            if (voucherCard) {
                if (newStatus) {
                    voucherCard.className = 'bg-white border border-slate-200 rounded-lg p-4 border-green-200 bg-green-50';
                } else {
                    voucherCard.className = 'bg-white border border-slate-200 rounded-lg p-4';
                }
            }
        }
        ...
        if (toggleElement) {
            const voucher = appState.vouchers.find(v => v.id == voucherId);
            const originalStatus = voucher ? voucher.is_active : false;
            
            const switchContainer = toggleElement.closest('.relative');
            const toggleBall = switchContainer.querySelector('div:last-child');
            const switchTrack = switchContainer.querySelector('div:first-child');
            
            toggleElement.checked = originalStatus;
            if (originalStatus) {
                switchTrack.className = 'w-10 h-6 rounded-full transition-colors bg-green-500';
                toggleBall.className = 'absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform translate-x-4';
            } else {
            ...
            const voucherCard = toggleElement.closest('.bg-white, .border-green-200');
            if (voucherCard) {
                if (originalStatus) {
                    voucherCard.className = 'bg-white border border-slate-200 rounded-lg p-4 border-green-200 bg-green-50';
                } else {
...
function handleDocumentClick(event) {
    ...
    if (target.className && target.className.includes('toggle-button-')) {
...
function setupEventListeners() {
    ...
    if (cartBtn) {
        cartBtn.addEventListener('click', openCart);
        console.log('✅ Cart button listener added');
    }
    ...
    if (navHomeBtn) {
        navHomeBtn.addEventListener('click', switchToHome);
        console.log('✅ Nav home button listener added');
    }
    
    const navAdminBtn = document.getElementById('nav-admin-btn');
    if (navAdminBtn) {
        navAdminBtn.addEventListener('click', switchToAdmin);
        console.log('✅ Nav admin button listener added');
    }
    
    const footerHomeBtn = document.getElementById('footer-home-btn');
    if (footerHomeBtn) {
        footerHomeBtn.addEventListener('click', switchToHome);
        console.log('✅ Footer home button listener added');
    }
    
    const footerAdminBtn = document.getElementById('footer-admin-btn');
    if (footerAdminBtn) {
        footerAdminBtn.addEventListener('click', switchToAdmin);
        console.log('✅ Footer admin button listener added');
    }
    ...
    if (applyVoucherBtn) {
        applyVoucherBtn.addEventListener('click', applyVoucher);
        console.log('✅ Apply voucher button listener added');
    }
    ...
    if (skipPromoBtn) {
        ...
        console.log('✅ Skip promo button listener added');
    }
...
// Admin functions - New reliable handlers
window.switchAdminTab = switchAdminTab;
window.showAddProductForm = showAddProductForm;
window.hideProductForm = hideProductForm;
window.editProduct = editProduct;
window.handleProductToggle = handleProductToggle;
window.handleVoucherToggle = handleVoucherToggle;
...
// Debug functions
window.debugToggleSwitches = debugToggleSwitches;
window.testProductToggle = testProductToggle;
window.testVoucherToggle = testVoucherToggle;

// Legacy support (deprecated but kept for compatibility)
window.toggleProductStatus = (productId) => {
    console.log('⚠️ LEGACY: toggleProductStatus called, redirecting to handleProductToggle');
    return handleProductToggle(productId);
};
window.toggleVoucherStatus = (voucherId) => {
    console.log('⚠️ LEGACY: toggleVoucherStatus called, redirecting to handleVoucherToggle');
    return handleVoucherToggle(voucherId);
};
...
console.log("🔧 Debug tools available: debugToggleSwitches(), testProductToggle(id), testVoucherToggle(id)");
