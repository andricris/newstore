...
function renderProductRow(product) {
    return `
        <tr class="border-b border-slate-100 hover:bg-slate-50">
            ...
            <td class="py-4 px-4">
                <div class="flex items-center">
                    <div class="toggle-switch-${product.id} flex items-center cursor-pointer" onclick="handleProductToggle(${product.id})">
                        <div class="relative inline-block w-12 h-6">
                            <div class="toggle-track w-12 h-6 rounded-full transition-colors duration-300 ${product.is_available ? 'bg-green-500' : 'bg-gray-300'}"></div>
                            <div class="toggle-thumb absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform duration-300 transform ${product.is_available ? 'translate-x-6' : 'translate-x-0'} shadow-md"></div>
                        </div>
                        <span class="toggle-label ml-3 text-sm font-medium ${product.is_available ? 'text-green-600' : 'text-gray-500'}">
                            ${product.is_available ? 'Tersedia' : 'Habis'}
                        </span>
                    </div>
                </div>
    ...
    return `
        <div class="bg-white border border-slate-200 rounded-lg p-4 ${isActive ? 'border-green-200 bg-green-50' : ''}">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-3">
                    <div class="toggle-switch-voucher-${voucher.id} flex items-center cursor-pointer" onclick="handleVoucherToggle(${voucher.id})">
                        <div class="relative inline-block w-12 h-6">
                            <div class="toggle-track w-12 h-6 rounded-full transition-colors duration-300 ${voucher.is_active ? 'bg-green-500' : 'bg-gray-300'}"></div>
                            <div class="toggle-thumb absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform duration-300 transform ${voucher.is_active ? 'translate-x-6' : 'translate-x-0'} shadow-md"></div>
                        </div>
                    </div>
...
async function toggleProductStatus(productId, toggleElement) {
    ...
    try {
        ...
        if (toggleElement) {
            ...
            // Update status text
            const statusText = toggleElement.closest('label').querySelector('span');
            if (statusText) {
                statusText.textContent = newStatus ? 'Tersedia' : 'Habis';
                statusText.className = `ml-2 text-sm ${newStatus ? 'text-green-600' : 'text-slate-500'}`;
            }
        }
        ...
        if (toggleElement) {
            const product = appState.products.find(p => p.id == productId);
            const originalStatus = product ? product.is_available : false;
            ...
            const statusText = toggleElement.closest('label').querySelector('span');
            if (statusText) {
                statusText.textContent = originalStatus ? 'Tersedia' : 'Habis';
                statusText.className = `ml-2 text-sm ${originalStatus ? 'text-green-600' : 'text-slate-500'}`;
            }
        }
    }
}
...
// Admin functions - Updated with new parameters
window.switchAdminTab = switchAdminTab;
window.showAddProductForm = showAddProductForm;
window.hideProductForm = hideProductForm;
window.editProduct = editProduct;
window.toggleProductStatus = (productId, toggleElement = null) => toggleProductStatus(productId, toggleElement);
window.deleteProduct = deleteProduct;
window.toggleDiscountInput = toggleDiscountInput;
window.toggleVoucherStatus = (voucherId, toggleElement = null) => toggleVoucherStatus(voucherId, toggleElement);
