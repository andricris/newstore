...
async function loadAllData() {
    ...
    try {
        ...
        // Update footer after loading data
...
/**
 * Update footer elements with the latest store information.
 * Pulls values from appState.settings and writes them to the DOM.
 */
function updateFooter() {
    ...
    if (addressEl) addressEl.textContent = store_address || 'Jl. Profesional No. 1 Jakarta, Indonesia 12345';
...
function showPromoPopup() {
    // Always show promo popup if media URL is set, regardless of previous sessions
    if (appState.settings.promo_media_url) {
...
function setupEventListeners() {
    ...
    if (skipPromoBtn) {
        skipPromoBtn.addEventListener('click', () => {
            ...
            // Do not persist promoShown; allow promo to appear on refresh
        });
    }
